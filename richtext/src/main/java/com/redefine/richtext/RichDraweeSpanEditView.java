 ...
 */
public class RichDraweeSpanEditView extends RichEditText implements IRichDraweeSpanView {

    IDraweeDelegateView mDraweeDelegate;
    private DraweeSpanChangeWatcher mSpanWatcher;
    private static final int CHANGE_WATCHER_PRIORITY = 100;

    public RichDraweeSpanEditView(Context context) {
        super(context);
    }

    public RichDraweeSpanEditView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public RichDraweeSpanEditView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private IDraweeDelegateView getDelegateView() {
        if (mDraweeDelegate == null) {
            mDraweeDelegate = RichDraweeViewFactory.createDraweeEditView(this);
        }
