 ...
 */
public class ProfileLikeViewModel extends AndroidViewModel implements UserLikePostsManager.UserLikePostsCallback {

    private String mUid;
    private boolean mIsBrowse;
    private String mFeedSource;
    private UserLikePostsManager mUserLikeManager;
    ...
    public ProfileLikeViewModel(@NonNull Application application) {
        super(application);
        mUserLikeManager = new UserLikePostsManager();
        mUserLikeManager.register(this);
    }
    ...
    public void refresh(String uid, boolean isBrowse, String source) {
        mUid = uid;
        mIsBrowse = isBrowse;
        mFeedSource = source;
        mUserLikeManager.tryRefreshPosts(uid, isBrowse);
        mPageStatus.postValue(PageStatusEnum.LOADING);
    }

    public void loadMore() {
        mUserLikeManager.tryHisPosts(mUid, mIsBrowse);
    ...
    public void onRefreshUserLikePosts(UserLikePostsManager manager, List<PostBase> posts, String uid, int newCount, int errCode) {
        if (mUserLikeManager != manager || !TextUtils.equals(uid, mUid)) {
            return;
        }
        ...
        PostEventManager.INSTANCE.setOldType(EventConstants.FEED_PAGE_OLD_PROFILE_LIKE_OWNER);
    ...
    protected void onCleared() {
        super.onCleared();
        mUserLikeManager.unregister(this);
    }
}
