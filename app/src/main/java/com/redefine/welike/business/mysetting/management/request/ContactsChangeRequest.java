...
public class ContactsChangeRequest extends BaseRequest {

    public ContactsChangeRequest(Context context) {
        super(BaseHttpReq.REQUEST_METHOD_PUT, context);
        Account account = AccountManager.getInstance().getAccount();
        if (account != null) {
            setHost("relationship/option/" + account.getUid() + "/notifyNewContact", true);
        }
    }
