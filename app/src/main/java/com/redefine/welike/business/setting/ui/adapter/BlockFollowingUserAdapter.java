...
import com.redefine.welike.business.setting.ui.viewholder.BlockFollowingUserHeadViewHolder;
import com.redefine.welike.business.setting.ui.viewholder.BlockFollowingUserItemViewHolder;
import com.redefine.welike.business.user.management.bean.User;

import java.util.ArrayList;
import java.util.List;

public class BlockFollowingUserAdapter extends LoadMoreFooterRecyclerAdapter<CommonTextHeadBean, User> {

    private final OnBlockUserOpCallback mCallback;
    private List<User> mUsers = new ArrayList<>();

    public BlockFollowingUserAdapter(OnBlockUserOpCallback callback) {
        mCallback = callback;
    }

    @Override
    protected BaseRecyclerViewHolder onCreateItemViewHolder(ViewGroup parent, int viewType) {
        return new BlockFollowingUserItemViewHolder(mInflater.inflate(R.layout.block_user_item, parent, false));
    }

    @Override
    protected BaseRecyclerViewHolder onCreateHeaderViewHolder(ViewGroup parent, int viewType) {
        return new BlockFollowingUserHeadViewHolder(mInflater.inflate(R.layout.block_user_head_title, parent, false));
    }
    ...
    protected void onBindItemViewHolder(BaseRecyclerViewHolder holder, int position) {
        ...
        if (holder instanceof BlockFollowingUserItemViewHolder) {
            ((BlockFollowingUserItemViewHolder) holder).itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (mCallback != null) {
                        mCallback.onBlock(user);
                    }
                }
            });
        }
    ...
    public static interface OnSearchUserClickListener {
