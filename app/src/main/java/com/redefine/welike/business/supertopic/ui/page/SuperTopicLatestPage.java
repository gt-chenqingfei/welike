...
import com.redefine.welike.business.supertopic.management.provider.SuperTopicLatestFeedsProvider;
...
public class SuperTopicLatestPage extends BaseLifecycleFragmentPage implements SinglePostsManager.PostsCallback, ILoadMoreDelegate, ErrorView.IErrorViewClickListener, IBrowseClickListener, OnPostButtonClickListener {
    ...
    public SuperTopicLatestPage(BaseLifecyclePageStackManager stackManager, PageConfig config, PageCache cache) {
    ...
    protected View createView(ViewGroup container, Bundle saveState) {
        return mPageStackManager.getLayoutInflater().inflate(R.layout.super_topic_latest_fragment_layout, null);
    }
    ...
    protected void initView(ViewGroup container, Bundle saveState) {
        ...
        recyclerView = getView().findViewById(R.id.recycler_latest);
...
//        recyclerView.addOnScrollListener(videoItemVisibilityCalculator);
        ...
        refreshLayout.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(RefreshLayout refreshlayout) {
                SuperTopicLatestPage.this.onRefresh();
        ...
        postModel.setDataSourceProvider(new SuperTopicLatestFeedsProvider(topicId));
        postModel.setListener(this);

        adapter = new SuperTopicRecylerViewAdapter(mPageStackManager, EventConstants.FEED_PAGE_SUPER_TOPIC_LATEST);
        if (isBrowse) {
        ...
        mPostViewTimeManager.attach(recyclerView, adapter, EventConstants.FEED_SOURCE_SUPER_TOPIC_LATRST);
    }
    ...
    public void onRefreshPosts(SinglePostsManager manager, List<PostBase> posts, int newCount, boolean last, int errCode) {
        ...
        PostEventManager.INSTANCE.setOldType(EventConstants.FEED_PAGE_OLD_SUPER_TOPIC_LATEST);
...
//            RegisterActivity.Companion.show(mPageStackManager.getContext(), 0, new RegisterAndLoginModel(EventLog.RegisterAndLogin.PageSource.OTHER));
    ...
    public void onCommentClick(PostBase postBase) {
        EventLog.Feed.report6(17, postBase == null ? "" : postBase.getPid(), PostEventManager.getPostType(postBase),
    ...
    public void onLikeClick(PostBase postBase, int exp) {
        EventLog.Feed.report5(postBase == null ? "" : postBase.getPid(),
                postBase == null ? 0 : postBase.getSuperLikeExp(), exp, 17,
    ...
    public void onForwardClick(PostBase postBase) {
        EventLog.Feed.report7(17, postBase == null ? "" : postBase.getPid(), PostEventManager.getPostType(postBase),
                postBase == null ? null : postBase.getStrategy(), postBase == null ? "" : postBase.getSequenceId());
    }
    ...
    public void onPostAreaClick(PostBase postBase, int clickArea) {
        EventLog.Feed.report8(EventConstants.FEED_SOURCE_SUPER_TOPIC_LATRST,
