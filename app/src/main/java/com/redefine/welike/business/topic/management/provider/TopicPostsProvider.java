...
import com.redefine.welike.business.browse.management.request.TopicFeedsRequest;
import com.redefine.welike.business.browse.management.request.TopicFeedsRequest2;
...
public class TopicPostsProvider extends PostsProviderBase implements ISinglePostsProvider, RequestCallback {
    ...
    public TopicPostsProvider(String id, boolean isBrowse) {
        tid = id;
    ...
    public void tryRefreshPosts() {
        ...
        cursor = null;
        cacheList.clear();
//        feedsRequest = new TopicFeedsRequest(MyApplication.getAppContext());
        try {
            if (!isBrowse) {
                feedsRequest = new TopicFeedsRequest(tid, null, MyApplication.getAppContext());
            } else {
                feedsRequest = new TopicFeedsRequest2(tid, null, MyApplication.getAppContext());
            }
...
//            feedsRequest = new TopicFeedsRequest(MyApplication.getAppContext());
            try {
                if (!isBrowse) {
                    feedsRequest = new TopicFeedsRequest(tid, cursor, MyApplication.getAppContext());
                } else {
                    feedsRequest = new TopicFeedsRequest2(tid, cursor, MyApplication.getAppContext());
                }
                feedsRequest.req(this);
            } catch (Exception e) {
                e.printStackTrace();
