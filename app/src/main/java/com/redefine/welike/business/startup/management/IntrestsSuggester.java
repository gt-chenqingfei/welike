...
public class IntrestsSuggester implements RequestCallback {
    ...
    public void refresh() {
        ...
        else  request = new IntrestsRequest(cursorNum, MyApplication.getAppContext(),false, AppsFlyerManager.getInstance().getReferrerId());
        try {
            request.req(this);
        } catch (Exception e) {
            e.printStackTrace();
