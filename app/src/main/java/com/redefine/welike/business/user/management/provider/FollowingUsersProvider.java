...
import com.redefine.welike.business.user.management.request.FollowingUsersRequest;
...
public class FollowingUsersProvider extends UsersProviderBase implements IUsersProvider, RequestCallback {
    private FollowingUsersRequest usersRequest;
    ...
    public void tryRefreshUsers(String uid) {
        ...
        index = 0;
        cacheList.clear();
        usersRequest = new FollowingUsersRequest(MyApplication.getAppContext());
        try {
    ...
    public void onSuccess(BaseRequest request, JSONObject result) throws Exception {
        if (request == usersRequest) {
            ...
            if (actionState == GlobalConfig.LIST_ACTION_REFRESH) {
                ...
                cacheFirstPage(list);
                if (listener != null) {
                    listener.onRefreshUsers(users, uid, newCount, ErrorCode.ERROR_SUCCESS);
                }
            } else if (actionState == GlobalConfig.LIST_ACTION_HIS) {
                callReceiveHisContacts(users, uid, ErrorCode.ERROR_SUCCESS);
            }
        }
    }
    ...
    private final String getUidFromRequest(FollowingUsersRequest request) {
        String uid = null;
        Object o = request.getUserInfo(FollowingUsersRequest.FOLLOWING_USERS_REQ_UID_KEY);
