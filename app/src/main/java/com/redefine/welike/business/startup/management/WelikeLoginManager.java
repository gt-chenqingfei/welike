...
import com.redefine.welike.business.startup.management.request.LoginRequest;
import com.redefine.welike.business.startup.management.request.ThirdLoginRequest;

/**
 * @author Created by honglin on 2018/6/13.
 */

public class WelikeLoginManager {
    ...
    public void reqLogin(String mobile, String SMSCode, LoginCallback callback) {
        ...
        request = new LoginRequest(mobile, SMSCode, MyApplication.getAppContext());
        try {
    ...
    public void reqFacebookLogin(String facebookToken, LoginCallback callback) {
        ...
        request = new ThirdLoginRequest(MyApplication.getAppContext());
        try {
            ((ThirdLoginRequest)request).req("1", facebookToken, new RequestCallback() {
    ...
    public void reqGoogleLogin(String googleToken, LoginCallback callback) {
        ...
        request = new ThirdLoginRequest(MyApplication.getAppContext());
        try {
            ((ThirdLoginRequest)request).req("2", googleToken, new RequestCallback() {
    ...
    public void reqTrueCallerLogin(String payload, String signature, String signatureAlgorithm, LoginCallback callback) {
        ...
        request = new ThirdLoginRequest(MyApplication.getAppContext());
        try {
            ((ThirdLoginRequest)request).req("3", payload, signature, signatureAlgorithm, new RequestCallback() {
                @Override
                public void onError(BaseRequest request, int errCode) {
                    handleError(request, errCode);
                }
