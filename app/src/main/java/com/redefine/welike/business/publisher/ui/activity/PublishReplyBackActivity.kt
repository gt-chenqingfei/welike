...
import com.redefine.welike.business.publisher.viewmodel.PublishReplyBackViewModel
import com.redefine.welike.commonui.util.ToastUtils
import com.redefine.welike.statistical.EventLog
import kotlinx.android.synthetic.main.activity_publish_reply_back.*
 ...
 */
@LayoutResource(layout = R.layout.activity_publish_reply_back, title = R.string.editor_comment_title)
class PublishReplyBackActivity : AbsPublishActivity<DraftReplyBack, PublishReplyBackViewModel>() {
    ...
    override fun getViewModel(): PublishReplyBackViewModel {
        val viewModel =
                ViewModelProviders.of(this).get(PublishReplyBackViewModel::class.java)
        viewModel.init(DraftCategory.COMMENT_REPLY_BACK, mDraftId)
        return viewModel
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        intent?.let { intent ->
            ...
            intent.getSerializableExtra(PublishReplyBackStarter.EXTRA_COMMENT)?.let {
                comment = it as Comment
            }
            intent.getSerializableExtra(PublishReplyBackStarter.EXTRA_RREPLY)?.let {
                reply = it as Comment
            }
            mViewModel.parseDraftReply(comment, reply)
        }
