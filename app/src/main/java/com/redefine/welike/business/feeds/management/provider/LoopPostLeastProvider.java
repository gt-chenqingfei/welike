 ...
 **/
public class LoopPostLeastProvider  extends PostsProviderBase implements ILoopLeastPostProvider, RequestCallback {
    private LoopRequest loopRequest;
    private String cursor;
    private LoopPostsLeastProviderCallBack listener;
    @Override
    public void onError(BaseRequest request, int errCode) {
        if (request == loopRequest) {
            loopRequest = null;
            if (listener != null) {
                listener.onGetNewLeastPosts(null, false, errCode);
            }
        }
    }
    ...
    public void onSuccess(BaseRequest request, JSONObject result) throws Exception {
        if (request == loopRequest) {
            ...
            cacheFirstPage(list);
            if (listener != null) {
                listener.onGetNewLeastPosts(list, TextUtils.isEmpty(cursor), ErrorCode.ERROR_SUCCESS);
            }
        }
    }
    ...
    @Override
    public void LoopGetNewLeastPost() {
        if (loopRequest != null) return;
        cursor = null;
        cacheList.clear();


        loopRequest = new LoopRequest(LoopRequest.LATEST_FEEDS, true, MyApplication.getAppContext());
        try {
            ...
            loopRequest = null;
            if (listener != null) {
                listener.onGetNewLeastPosts(null, false, ErrorCode.networkExceptionToErrCode(e));
            }
        }
    }
    ...
    @Override
    public void setListener(LoopPostsLeastProviderCallBack callback) {
