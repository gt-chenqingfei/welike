...
public class SessionCommentViewHolder extends BaseSessionItemViewHolder<SessionModel> {
    ...
    public SessionCommentViewHolder(View itemView, SessionItemViewHolder.IDeleteSessionCallback listener) {
        super(itemView);
        mCallback = listener;
        mCommentText = ResourceTool.getString(ResourceTool.ResourceFileEnum.IM, "message_comment_text");
    ...
    public void bindViews(RecyclerView.Adapter adapter, final SessionModel imSession) {
        if(!(imSession instanceof CommentSessionModel)) {
            return;
        }
        mSessionName.setText(mCommentText);
        mSessionHeader.setImageResource(R.drawable.session_comment_icon);
        int unreadCount = ((CommentSessionModel) imSession).getUnreadCount();
        if (unreadCount > 0) {
            IMEventManager.INSTANCE.setComment_num(Math.min(unreadCount, 99));
            mUnreadCount.setVisibility(View.VISIBLE);
            mUnreadCount.setText(unreadCount > 99 ? "99+" : String.valueOf(unreadCount));
        } else {
            IMEventManager.INSTANCE.setComment_num(0);
